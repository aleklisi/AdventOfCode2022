...
execute_command({HowMany, From, To}, Stacks) ->
    OldFromStack = maps:get(From, Stacks),
    OldToStack = maps:get(To, Stacks),
    {PartToBeMoved, NewFromStack} = lists:split(HowMany, OldFromStack),
    NewToStack = PartToBeMoved ++ OldToStack,
    NewStacks1 = maps:put(From, NewFromStack, Stacks),
    maps:put(To, NewToStack, NewStacks1).
